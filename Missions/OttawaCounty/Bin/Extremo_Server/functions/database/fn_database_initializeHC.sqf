...
if(count _headlessclients >= 1)exitWith
{
	...
	extdb_var_database_headless_clientconnected = addMissionEventHandler [
		'PlayerConnected',
		{
			...
			format["Headless client [%1] avaliable", _steamID] call Extremo_fnc_database_systemlog;
	...
	extdb_var_database_headless_clientdisconnected = addMissionEventHandler [
		"PlayerDisconnected",
		{
			...
			if !(_isHeadless)exitWith{true};
			format["Headless client [%1] left (OR) lost connection. HC is no longer avaliable", _steamID] call Extremo_fnc_database_systemlog;
	...
	//--- Log
	{
		format["Waiting for headless client [%1] to connect", _x] call Extremo_fnc_database_systemlog;
	} forEach _headlessclients;

	true
};

"No headless clients defined" call Extremo_fnc_database_systemlog;
