...
try {
	...
	if(_versionRes < 1.031) throw "Error with extDB3";
 	format ["ExtDB V%1 Loaded",_versionRes] call Extremo_fnc_database_systemlog;
	...
	format ["Profile (%1) Loaded",_profile] call Extremo_fnc_database_systemlog;
	...
	format ["SQL%1 Protocol Loaded",["Raw","Custom"]select _sqlcustom] call Extremo_fnc_database_systemlog;
	...
	format ["Profile (%1) Locked",_profile] call Extremo_fnc_database_systemlog;
	 
	//--- Connection OKAY
	extdb_var_database_prepared = compileFinal str(_sqlcustom);
	extdb_var_database_key = compileFinal str(_databasekey); 
	format["Database Connected Using Profile: (%1)", _profile] call Extremo_fnc_database_systemlog;

	//--- Setup server to support headless clients
	if (_headless) then {
		[] call Extremo_fnc_database_initializeHC;
	};

	//--- Run stored procedures for SQL side cleanup
	{
		["CALL", _x]call Extremo_fnc_database_request;
		format["Executing procedure (%1)", _x] call Extremo_fnc_database_systemlog;
	} forEach _procedures;
} catch { 
