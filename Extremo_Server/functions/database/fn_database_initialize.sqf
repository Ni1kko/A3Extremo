...
//--- Reload variables from uiNamespace into missionNamespace or set default into missionNamespace
if(isNil "extdb_var_database_key")then{
	_reconnected = (DLL callExtension "9:LOCK_STATUS" isEqualTo "[1]");
	{missionNamespace setVariable [_x#0,uiNamespace getVariable [_x#0, _x#1]]}forEach [
...
if(extdb_var_database_version isEqualTo "") exitWith {
	...
	[missionNamespace,["extdb_var_database_error",true]] remoteExec ["setVariable", 0, "extdb_var_database_errorJIP"];
};
...
if _reconnected exitWith {
	...
	[missionNamespace,["extdb_var_database_error",false]] remoteExec ["setVariable", 0, "extdb_var_database_errorJIP"];
